...
base_marginacion2 <- base_marginacion #Guardo una copia del objeto original porque habrá efectos colaterales. 

#Opción 1. Un bucle. Solución en R MUY base. Dos malas prácticas de programación en una: bucles y efectos secundarios. 

for(i in 1:nrow(base_marginacion)){           #i es el índice, 1:nrow es una secuencia de enteros de 1 hasta el número de filas de la base de datos (14641)
  if (base_marginacion$ENT[i] %in% Noroeste){  #Si base_marginacion en la fila i está en Noroeste...
    base_marginacion$REG[i] = "Noroeste"}      #base_marginacion$REG en la fila i tendrá el valor "Noroeste"
  if (base_marginacion$ENT[i] %in% Noreste){   #Y así hasta el final...
...
base_marginacion %>% group_by(REG, ENT) %>% tally() %>% select(-n) %>% kable()
```
...
base_marginacion <- base_marginacion2 #Regreso a la base orginal, ya que el bucle la modificó.

#Ejecución de case_when()
#========================
...
base_marginacion %>% 
  ...
  group_by(REGION, ENT, CVE_ENT) %>% 
  tally()
```

No sólo es mucho más rápido y claro, también es posible case_when() dentro del paradigma de programación funcional. La función tiene un input  (la bases de datos y los vectores que señalan qué Entidad pertenece a cada región) y un output (la nueva columna REG). Puedo usar ese output en la operación siguiente y nunca modificar el data.frame original. No hay forma de corromper la base de datos y no es necesario hacer copias y revertir los cambios por efectos colatrerales, como tuvimos que hacer con el bucle. 

### Opción 2.1. Función vectorizada con `ifelse()`

Otra alternativa de función vectorizada `ifelse()`. De hecho `case_when()` es una versión especial de `ifelse()` con sintaxis simplificada para recodificaciones. `ifelse()` es una función genérica para evaluar condiciones, más flexible que `case_when()` en cuanto a su aplicación, pero con una sintaxis más intrincada, ya que el ámbito de cada `ifelse()` anidado se delimita con paréntesis que no se cierran inmediatamente. Por eso es conveniente organizar el código de modo tal que los paréntesis de cierre esten en la misma columna que los de apertura. 

```{r}
#Opción2.1. (con código de Ulises Sánchez)
#=========================================

base_marginacion %>% 
  mutate (
    REGION=ifelse (
      ENT %in% Centro_norte, "Centro_norte", ifelse (
        ENT %in% Centro_sur, "Centro_sur", ifelse (
          ENT %in% Este, "Este", ifelse (
            ENT %in% Noreste,"Noreste", ifelse (
              ENT %in% Noroeste, "Noroeste", ifelse (
                ENT %in% Oeste, "Oeste", ifelse (
                  ENT %in% Sur_este, "Sur_este", "Sur_oeste") #Cierra el último ifelse.
                                                     )
                                               )
                                         )
                                                  )
                                                    )
                             )                                #Cierra el primer ifelse.
             ) %>%  
  group_by(REGION, ENT, CVE_ENT) %>% 
  tally()  
```

En ambos casos logramos el resultado correcto. Sin embargo dependemos de 8 vectores para hacer que el código funcione. Tiene que haber una manera mejor. 
...
kable(tabla_regiones, caption="Tabla de empate entre Regiones, Entidades y clave de entidad.")
...
system.time(for(i in 1:nrow(base_marginacion)){           #i es el índice, 1:nrow es una secuencia de enteros de 1             hasta el número de filas de la base de datos (14641)
        ...
        ENT %in% Centro_sur, "Centro_sur", ifelse (
          ENT %in% Este, "Este", ifelse (
            ENT %in% Noreste,"Noreste", ifelse (
             ...
             ) %>% system.time() ->opcion2_1

rbind(opcion1, opcion2, opcion2_1, opcion3) %>% as.data.frame %>% kable(., caption="Tiempos de ejecución con resolución en centésimas de segundo.")
```

Con más de 7 segundos de tiempo de ejecución el bucle es la peor opción. case_when e inner_join son tan rápidos que la función `Sys.time()` no reporta sus tiempos de ejecución por ser menores a 0.01 segundos. Una prueba de desempeño más detallada arroja 8 nanosegundos para case_when y 20 nanosegundos para inner_join y 16.5 nanosegundos para `ifelse()`.  
Es decir, en el tiempo en que se ejecuta case_when para esta base de datos en esta computadora un fotón recorre 2.4 metros, con ifelse 5, con inner_join 6 metros y para el bucle entre un millón y medio y dos millones de kilómetros, es decir, a mitad de camino de la luna.  
...
`case_when()`, `ifelse()` e `inner_join()` están cercanos en velocidad, así que tendremos que usar otro criterio para seleccionar. Me inclino por usar un join. 

- Tiene una sintaxis más corta y simple que case_when o ifelse (menos sintaxis, menos errores).
