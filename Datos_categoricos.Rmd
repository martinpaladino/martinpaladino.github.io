...
rexthor[1:2, 1:2] <- NA
names(rexthor) <- names(x)                     #Perdió los nombres al coercinarla a matríz. 
pander(rexthor, caption="La tabla robada")
```
...
```{r}
espe <- esperados(x)
dimnames(espe) <- dimnames(x)
pander(espe, caption="Frecuencias esperadas bajo el supuesto de independencia estadística")
```
