...
subtitle: "con dplyr, tidyr y tibble"
...
tribble(
  ...
  rename(Tipo=Col1,              
         Fruta=Col2,             
         Precio=Col3) %>% 
  filter(Col2=="Cítrico")        #filter() regresa un error: Col2 no existe. ¿Por qué? Porque la renombramos el paso anterior 
...
tribble(
  ...
  rename(Tipo=Col1,              
         Fruta=Col2,             
         Precio=Col3) %>% 
  filter(Precio>=12)    #Usando >= conservo solamente las filas en las que el precio es mayor a igual a 12. <, >, y <= también son válidos. zs
...
`filter()` utiliza todos los operadores lógicos de R, incluidos `&`, `|` e `%in$`. `&` es y y lo usamos para unir dos condiciones que deben cumplirse para que una fila sea conservada. `|` es o, lo usamos cuando pueden cumplirse una condición *o* la otra. %in% en operaciones de conjuntos equivale a pertenece. Busca igualdad, del mismo modo que `==`, sólo que lo hace con respecto a un conjunto --en R típicamente será un vector-- y no a una cadena. Un operador que no pertenece a `dplyr::` o `base::`pero que también puede ser de gran utilidad es `%like%`, que importamos del paquete `data.table::`. `like::` permite especificar igualdades parciales.   
...
#Cadenas similares
filter(municipios, Municipio %like% "Juárez" & Población <400000)        #Incluye Unión Juárez, similar a "Juárez"

#Una u otra condición.
filter(municipios, Municipio =="Juárez" | Población==137927)             #Municipios llamados Juárez o con una población de 137927
...
municipios %>%
  mutate (acumulado = cumsum(Población), diferencia=Población-lag(Población)) %>% 
  mutate_if(is.numeric, funs(paste("Son", . , "personas")))                        #Aplica la misma transformación a todas las variables numéricas.
municipios %>% mutate_all(as.character)                                            #Transforma a todas las colunas al tipo caracter.
```
##Sumarios avanzados. 

`summarise()` nos permite crear sumarios y es especialmente útil con datos agrupados. Sin embargo es posible que no exista una función que cree el sumario que necesitamos. Podemos pasar a `summarise()` una función personalizada. En este caso estimaremos la media de población para nuestra muestra, el desvío estandar y el error estandar de la media: $SE\hat{x}=\frac{\sigma}{\sqrt{n}}$

```{r summarise_avanzado}
summarise(municipios, n=n(), media=mean(Población), desvio=sd(Población), error_estandar=sd(Población)/sqrt(sum(Población)))
...
municipios %>% 
  ...
  summarise(n=n(), 
            ...
            desvio=sd(Población), 
            error_estandar=sd(Población)/sqrt(sum(Población))) #NaN: Not a Number. No se puede calcular desvío y error estandar con n=1.  
...
inner_join(frutería1, frutería2, by = "Fruta")  #Une las filas en las que coinicen frutería1 y frutería2, usando Fruta como clave. Sólo conserva las coincidencias, mandarina desaparece porque no tiene precio futuro.

full_join(frutería1, frutería2, by = "Fruta")   #Une todas las filas, imputa NA cuando hay información faltante. No precio futuro para Mandarina y falta información para Toronja.

anti_join(frutería1, frutería2, by = "Fruta")   #Conserva sólamente las filas frutería1 que no coinciden con frutería2. 

anti_join(frutería2, frutería1, by = "Fruta")   #Conserva las filas de frutería2 que no cinciden con frutería1.

left_join(frutería1, frutería2, by = "Fruta")   #Conserva sólo las filas de frutería1 y agregas las columnas de frutería2, imputa NA si no hay coincidencia. Hay mandarina, pero no toronja...
```

##Operadores avanzados de `magrittr::`

Ya hemos utilizado el principal operador de `magrittr::`, la tubería `%>%` que estádisponible directamente en `dplyr::`. Entender el funcionamiento de `%>%`  puede ayudarnos a conocer sus límites y buscar alternativas para solucionar algunos problemas. En términos técnicos `%>%` es un operador binario, de la misma familia de `+` para sumar o `-` para restar. Son binarios porque se ubican entre dos expresiones y establecen una relación entre ellas. A las dos expresiones que envuelven a un operador binario las llamados *lado derecho* y *lado izquierdo*. Así, en la resta o en la división es de gran importancia ese orden, no es lo mismo `10-2` que `2-10`. Con los operadores de magrittr pasa exactamente lo mismo, la expresión que está del lado derecho pasa su output a la del lado izquierdo. R no contempla un método específico para pasar los datos de una función a la siguiente, pero el autor de `magrittr::` encontró una manera ingeniosa. Dado que la mayoría de las funciones toma a los datos como primera argumento `%>%` se encarga de "reescribir" la función del lado izquierdo, de manera el primer argumento sean los datos que produjo el lado derecho de la formula. Esto funciona en la mayoría de los casos, pero podemos tener problemas de la función que estamos utilizando en el lado izquierdo espera algo diferente a los datos como primer argumento. Asi con los modelos lineales que especificamos con `lm()`, esperan como primer argumento de la función la formula del modelo, no los datos. Para solucionar estos problemas `magrittr::` utiliza al signo `.` (un punto) como "comodín" para los datos y nos permite ubicarlos allí donde la sintaxis de la función del lado izquierdo los requiera. 
...
municipios %>% 
  mutate(Pob2 = Población*2) %>%         #Creo una nueva variable numérica, doble de la población. 
  lm(Pob2~Población, data=.) %>%         #Genero el modelo y lo paso a la derecha.
  summary()                              #Obtengo el sumario del modelo. ^2=1
...
Otros problemas son tecnicos y demandan mucha flexibilidad y creatividad al momento de escribir código. Para mantenernos en el paradigma de programción funcional debemos buscar formas diferentes de lograr los mismo. 
...
#Agregamos cada línea necesaria para la limpieza. Al interior de la función asignamos con =, de modo de no cargar de objetos al entorno global. 
...
base_marginacion %>% 
  ...
  ggplot(aes(valor)) +
  geom_bar() + 
  facet_wrap(~Variable,scales="free") + theme(axis.text = element_text(angle=90))

#Relaciones entre variables numéricas. chart.Correlation ftw! 
