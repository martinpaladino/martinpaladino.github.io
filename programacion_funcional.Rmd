...
output:
  html_document:
    ...
    theme: united
    toc: yes
subtitle: "Aplicando el paradigma de programación funcional"
lang: es-ES
---
...
```{r cadena_de_software, fig.cap="Cadena de software libre para la investigación reproducible", echo=FALSE, warning=FALSE, message=FALSE}

library(DiagrammeR)
grViz("
digraph cadena_reproducible {
  graph [overlap=false]
subgraph cluster_0 {  
node [shape = tab, fontname = Helvetica]
  C[label='Fuentes de datos']
  D[label='Rmarkdown']
  G[label='.html']
  H[label='.docx']
  I[label='.tex']
...
label='Cadena de software libre \npara investigación reproducible'
fontname=Helvetica
A->D B->D C->D D->E E->F F->{G, H, I} I->L L->J G->N[dir=none]
            }
subgraph cluster_1 {
...
```{r rversusexcel, fig.cap="Curva de aprendizaje y dificultad de problemas.", warning=FALSE, message=FALSE, echo=FALSE}
library(tidyverse)
tribble(~prog,  ~x, ~y,
        "Excel",  0, 0, 
        "Excel",  1, 0, 
...
ls()                          #Muestra el entorno de trabajo en consola.

rm(list=ls())                 #Borro el entorno de trabajo completo.
...
grViz("digraph boxes_and_circles {
  ...
  node [shape = circle,
        fontname = Helvetica, label ='Datos']
  A, C
    node [shape = circle,
        fontname = Helvetica]
  D [label ='Datos\nrecodificados\n2']
...
subgraph cluster_1 { 
  ...
  11->D
  D->{9,10}
    E->10[style=dotted, label=' ¿O son\nestos\ndatos\nlos que\n necesito?']
  E->10[style=dotted, label='', color='white']
  label='Programación procedimental'
  fontname=Helvetica
  fontsize = 14
                }
}
...
Las **funciones puras** son aquellas que reciben un input y nos regresan un output y no producen efectos colaterales. Un efecto colateral es un efecto de la función que produce un cambio de estado del programa fuera del ámbito estricto esa función.^[Evidentemente no consideramos al output de la función como un efecto colateral.] Dado que no queremos cambiar estados más allá del input-output de la función y que frecuentemente el resultado final esperado implica más de una función sería una buena idea poder encadenar funciones, de modo que el output de una sea el input de la siguiente. Para esto nos ayuda la transparencia referencial. 
...
#Programación procedimental. Con efectos colaterales.  
...
- Por su paz mental nunca recicle nombres: un objeto distinto requiere un nombre distinto. Nunca debería crear líneas del tipo `datos <- datos[algo]`, `datos_algo <- datos[algo]` es una mejor alternativa.  
