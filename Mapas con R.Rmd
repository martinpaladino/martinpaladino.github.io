...
```{r}
#Arreglar la manipulación de datos, funciona pero tiene es un abuso de <- y $. Hacerla funcional y menos fea. 

#Función de este cuate para mover un vector una fila hacia arriba. Sirve p calcular la diferencia de tiempo entre datapoints. Alerta: ¡copypasta!
...
bici <- as.data.frame(readGPX("19_feb._2017_10_54_56.gpx")$tracks) #Cargo la ruta y la paso a df.  

names(bici) <- c("lon", "lat", "ele", "time", "extensions") #Le arreglo los nombres.

ggplot(bici, aes(x=lon, y=lat)) +  geom_point() +  theme_minimal() #Prueba de puntos. 

#Más copypasta de este cuate.

bici$time <- strptime(bici$time, format = "%Y-%m-%dT%H:%M:%OS")   #Pasa time a fecha y hora POSIXt
bici$time.p1 <- shift.vec(bici$time, -1)                          #Crea una vector nuevo, desplazado una fila.  
bici$diferencia <- as.numeric(difftime(bici$time.p1, bici$time))  #Regresa la diferencia en segundos entre datapoints. 
bici$acum <- round(cumsum(bici$diferencia)/60, 0)                 #Ojo! por el shift asigna un NA al final.

#obtengo un mapa de la zona en google. 
chilango <- get_map (location=c(-99.16, 19.425), source="google", maptype="terrain", zoom=12) 

#Alerta de código orrible 2: seleccionar un data point cada 50, else NA. 

vector <- seq(1, 1662, 50)
bici$tiempo <- ifelse (row_number(bici$acum) %in% vector, bici$acum, NA)

#Genero el mapa. 
ggmap(chilango) + geom_point(data=bici, aes(x=lon, y=lat), size=.5) +  
  geom_label_repel(data=bici, aes(x=lon, y=lat, label=tiempo))
```
...
series_produccion_autos(token) %>% 
  ...
  geom_line() + 
 # stat_peaks(ignore_threshold = 0.4, strict=T) + 
 # stat_valleys(ignore_threshold = 0.4, strict=T) + 
    stat_peaks(geom = "text", angle = 90,hjust = -0.1, x.label.fmt = "%b%y",  ignore_threshold = 0.4, strict = T) + stat_valleys(geom = "text", angle = 90,hjust = 1.5, x.label.fmt = "%b%y",  ignore_threshold = 0.4, strict = T) + stat_smooth() + 
